...
### 1.1 ⚡ PUSH - АВТОМАТИЧЕСКАЯ ПРОВЕРКА!

**ВАЖНО:** Перед каждым `git push` автоматически запускается **pre-push хук**!
Он проверяет:

- ✅ Formatting (Prettier) — авто-исправляет если есть ошибки
- ✅ TypeScript — блокирует пуш при ошибках
- ✅ Build — блокирует пуш если билд падает
- ✅ YAML файлы — проверяет валидность
- ✅ package.json — проверяет JSON синтаксис

**Как пушить безопасно:**

```bash
# Способ 1: Обычный git push (pre-push сработает автоматически)
git add -A
git commit -m "chore: описание изменений"
git push                    # ← Автоматическая проверка!

# Способ 2: Использовать safe-push (более детальный вывод)
bun run safe-push           # ← Проверка + push вместе

# Способ 3: Только исправить, не пушить
bun run fix-only            # ← Исправить formatting и т.д.

# Только в исключительных случаях (не рекомендуется!):
git push --no-verify        # ← Пропустить проверки
```

**Если pre-push заблокировал push:**

```bash
# 1. Посмотрите какие ошибки
bun run fix-only            # Показывает детали ошибок

# 2. Исправьте вручную или автоматически
npm run format              # Форматирование
npx tsc --noEmit            # TypeScript ошибки
bun run build               # Build проверка
