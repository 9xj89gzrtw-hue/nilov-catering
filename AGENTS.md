...
> **⚠️ КРИТИЧНО ВАЖНО:** Выполните `bun run start-work` ПЕРЕД любой работой!
> Это ОБЯЗАТЕЛЬНЫЙ скрипт инициализации. Без него работа не начнётся.

---

## 🚨 ПЕРВЫЙ ШАГ - ИНИЦИАЛИЗАЦИЯ (ОБЯЗАТЕЛЬНО!)

```bash
# ЗАПУСТИТЕ ЭТО ПЕРЕД ЛЮБОЙ РАБОТОЙ:
bun run start-work

# Этот скрипт:
# ✅ Проверит наличие всех файлов документации
# ✅ Покажет какие файлы нужно прочитать
# ✅ Создаст маркер что вы ознакомились с правилами
# ⚠️  БЛОКИРУЕТ работу если нет обязательных файлов
```

**Если вы не запустили start-work - ВЫ НАРУШАЕТЕ ПРАВИЛА!**

---

## 📚 ДОКУМЕНТАЦИЯ (ОБЯЗАТЕЛЬНАЯ ДЛЯ ЧТЕНИЯ)

| Файл                      | Когда читать  | Что внутри                   |
| ------------------------- | ------------- | ---------------------------- |
| **DESIGN-SYSTEM.md**      | 🔄 КАЖДЫЙ РАЗ | Цвета, типографика, спейсинг |
| **COMPONENTS-CATALOG.md** | 🔄 КАЖДЫЙ РАЗ | Готовые компоненты           |
| **CODE-PATTERNS.md**      | 🔄 КАЖДЫЙ РАЗ | Паттерны кода                |
| **ERRORS-CHEATSHEET.md**  | При ошибке    | Решения проблем              |
| **REVIEW-CHECKLIST.md**   | Перед пушем   | Чеклист качества             |

### 🎯 САМОЕ ВАЖНОЕ ПРАВИЛО:

**НЕ СОЗДАВАЙТЕ КОД С НУЛЯ — КОПИРУЙТЕ ИЗ ДОКУМЕНТАЦИИ!**

❌ Неправильно: "Напишу компонент карточки с нуля"
✅ Правильно: "Открою COMPONENTS-CATALOG.md → скопирую Card pattern"

❌ Неправильно: "Использую цвет #fff"
✅ Правильно: "Открою DESIGN-SYSTEM.md → использую bg-white"

❌ Неправильно: "Создам форму как умею"
✅ Правильно: "Открою CODE-PATTERNS.md → скопирую Form pattern"
