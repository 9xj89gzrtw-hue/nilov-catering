...
export default function CollectionEditor({ params }: Props) {
  ...
  const updateRow = (idx: number, key: string, value: unknown) => {
    setData((prev) =>
      prev
        ? prev.map((row, i) =>
            i === idx ? { ...(row as Record<string, unknown>), [key]: value } : row
          )
        : null
    );
  };
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-800">
              {fields.map((f) => (
                <th key={f} className="px-3 py-2 text-left font-medium text-gray-400">
                  {f}
                </th>
              ))}
                      ...
                      <td key={f} className="px-3 py-1.5">
                        {typeof val === "boolean" ? (
                          <input
                            type="checkbox"
                            checked={val}
                            onChange={(e) => updateRow(idx, f, e.target.checked)}
                            className="accent-emerald-500"
                          />
                        ) : typeof val === "object" ? (
                          <input
                            className="w-full rounded border border-gray-600 bg-transparent px-2 py-0.5 font-mono text-xs"
                            value={JSON.stringify(val)}
                            onChange={(e) => {
                              try {
                                updateRow(idx, f, JSON.parse(e.target.value));
                              } catch {}
                            }}
                          />
                        ) : (
                          <input
                            className="w-full rounded border border-gray-600 bg-transparent px-2 py-0.5"
                            value={
                              typeof val === "string"
                                ? val
                                : typeof val === "number"
