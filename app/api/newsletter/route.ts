...
export async function POST(request: Request) {
  try {
    ...
    // Subscription persisted; log only on failure for debugging
    if (!fileOk && !telegramOk) {
      console.error(
        `[NEWSLETTER] Delivery failed for ${email} — check TELEGRAM env vars. Email logged.`
      );
    }
...
export async function GET() {
  return NextResponse.json(
    { success: false, message: "Используйте POST для подписки" },
