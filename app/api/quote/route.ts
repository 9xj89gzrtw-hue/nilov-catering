...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Lead is persisted via file/Telegram and logged server-side (Vercel captures console.error)
    if (!fileOk && !telegramOk) {
      console.error(
        `[QUOTE] Lead ${orderId} delivery failed — check TELEGRAM_BOT_TOKEN/TELEGRAM_CHAT_ID env vars. Lead logged above.`
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message:
        "Заявка принята. Мы свяжемся с вами в течение 15 минут в рабочее время (9:00–21:00).",
      orderId,
...
export async function GET() {
  return NextResponse.json(
    { success: false, message: "Используйте POST для отправки заявки" },
