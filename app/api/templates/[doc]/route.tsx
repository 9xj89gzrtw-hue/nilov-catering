...
function DocFooter() {
  return (
    <View style={styles.footer} fixed>
      ...
      <Text>
        {SITE.phone} · {SITE.email} · odaeda.ru · стр. {`$PAGE/$TOTAL_PAGES`}
      </Text>
  ...
  return (
    <Document title="Договор на оказание кейтеринговых услуг — NiloV Catering" author={SITE.legalName}>
      <Page size="A4" style={styles.page}>
        ...
        <Text style={styles.p}>
          3.3. ИП Нилов Д.И. применяет УСН (без НДС). Для Заказчиков на ОСН, требующих счёт-фактуру с НДС, работы через партнёрское ООО «ОдаЕда» (ИНН 7816583688, ОГРН 1147847107650).
        </Text>
        <Text style={styles.p}>
