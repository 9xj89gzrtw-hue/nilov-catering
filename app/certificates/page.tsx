...
export const metadata: Metadata = {
  ...
  title: 'Сертификаты, безопасность и документы — NiloV Catering',
...
const CERTIFICATES = [
  ...
  },
  {
    ...
    status: 'Действует · номер по запросу',
    statusColor: 'gold',
    details: [
      ...
      'Срок действия: 3 года, перевыставляется.',
      'Номер декларации и скан предоставляются по запросу на email.',
    ],
    note: 'Для получения скана декларации напишите на info@odaeda.ru — отправим в течение 30 минут в рабочее время.',
  },
  {
    icon: '🍚',
  ...
  },
  {
    ...
    status: 'По запросу · сертифицирующий орган',
    statusColor: 'gold',
    details: [
      'Халяль-меню готовится по стандартам Совета муфтиев России (ДУМ РФ).',
      'Забой: по обряду зибха (zibh) с произнесением такбира (tasmiya).',
      'Поставщики мяса: сертифицированные халяль-бойни Ленинградской области.',
      'Раздельное оборудование: отдельные мангал, гриль, посуда, ножи, доски — без пересечения со свининой.',
    ...
    note: 'Сертификат халяль от Совета муфтиев России или Международного центра стандартизации и сертификации «Халяль» — предоставляется по запросу. Укажите необходимость при заявке.',
  },
  {
    icon: '🌾',
    ...
    details: [
      ...
      'Отдельная зона кухни: отдельные разделочные столы, отдельные ножи/доски (синяя маркировка).',
  ...
  },
  {
    ...
    details: [
      ...
      'Счёт на оплату — для юр.лиц (безналичный расчёт, с НДС или без НДС по соглашению).',
      'Акт выполненных работ — подписывается после события.',
      'Счёт-фактура — для плательщиков НДС (при работе на ОСН).',
