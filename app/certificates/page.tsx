...
export const metadata: Metadata = {
  alternates: {
    ...
    languages: { ru: "/certificates", "x-default": "/certificates" },
  },
  title: "Сертификаты, безопасность и документы",
  description:
    "Полный пакет документов: ИНН/ОГРНИП, ТР ТС 022/2011, HACCP, медицинские книжки, журнал бракеража, программа производственного контроля, страхование ответственности.",
};
...
const CERTIFICATES = [
  {
    ...
    details: [
      ...
      `Примечание: Дмитрий Нилов ведёт кейтеринговую деятельность с 2007 года. До 2014 года — как частный повар и по найму в ресторанах СПб. Регистрация ИП в 2014 году оформлена для работы с юридическими лицами и тендерами.`,
      `Юридический адрес: ${SITE.legalAddress}`,
      `Email: ${SITE.email}`,
      `Телефон: ${SITE.phone}`,
      `Банк: ${"Филиал №7801 ПАО «Банк ВТБ» в г. Санкт-Петербурге"}`,
      `БИК: ${"044030706"}`,
      `К/с: ${"30101810200000000706"}`,
      `Р/с: предоставляется в счёте после заключения договора`,
    ],
  ...
  },
  {
    ...
    status: "Утверждена, действует",
    statusColor: "success",
    details: [
      "Программа производственного контроля — утверждена, ведётся с 2014 года.",
      "Покрывает: санитарно-гигиенический контроль производства, транспорта, персонала.",
  ...
  },
  {
    ...
    details: [
      ...
      "Обучение поваров: ежеквартальный инструктаж по аллергенам и перекрёстного загрязнения.",
    ],
  ...
  },
  {
    ...
    details: [
      ...
      "Счёт на оплату — для юр.лиц (безналичный расчёт). ИП Нилов Д.И. применяет УСН (без НДС). Для плательщиков НДС (ОСН) оформляем через партнёрское ООО. Полные реквизиты — на странице Контакты.",
      "Акт выполненных работ — подписывается после события.",
      "Счёт-фактура — через партнёрское ООО (по запросу). Уточните при заявке.",
  ...
  },
  {
    ...
    details: [
      ...
      "Базовый лимит — 5 000 000 ₽ на одно событие (ведущие страховые компании РФ).",
  ...
  },
  {
    ...
    statusColor: "gold",
    details: [
      "Для событий на 500+ гостей работаем через сертифицированные кухонные мощности.",
    ...
    note: "Запросить список сертифицированных кухонь и их ИНН/сертификаты — b2b@nilov-catering.ru (после подписания NDA). Пример фестивального кейса на 800 чел × 2 дня — /why-us (комбинированный кейс).",
  },
];
    ...
    <main id="main" className="pt-24 pb-20">
      <div className="container-site max-w-4xl">
        ...
        <p className="text-muted-foreground mb-8 text-lg">
          Полный пакет документов для B2B-заказчиков, школ, учреждений, частных клиентов. Реквизиты
          юридического лица опубликованы — проверка контрагента доступна онлайн. Цифровые шаблоны
          (договор, NDA, SLA) доступны для прямого скачивания ниже. Сканы сертификатов и медкнижек —
          по запросу в течение 1 рабочего дня.
        </p>

        {/* Quick legal block */}
        <div className="border-gold-tint bg-gold-tint/5 mb-10 rounded-xl border-2 p-6">
          <h2 className="font-heading mb-3 text-xl font-medium">Реквизиты — резюме</h2>
        ...
        <div className="grid gap-5 md:grid-cols-2">
          {CERTIFICATES.map((cert) => (
            <div key={cert.title} className="border-line bg-card rounded-xl border p-5">
              <div className="mb-3 flex items-start gap-3">
                <span className="text-3xl" aria-hidden="true">
                  {cert.icon}
                </span>
                <div className="flex-1">
                  <h2 className="font-heading mb-1 text-base font-medium">{cert.title}</h2>
                  <span
                    className={`inline-block rounded-full px-2 py-0.5 text-xs font-semibold ${
                      cert.statusColor === "success"
                        ? "bg-success/15 text-success"
                        : cert.statusColor === "gold"
              ...
              <ul className="text-muted-foreground mb-3 space-y-1.5 text-sm">
                {cert.details.map((d, i) => (
                  <li key={i}>{d}</li>
                ))}
              </ul>
              {cert.note && (
                <p className="text-muted-foreground border-line/40 mt-3 border-t pt-3 text-xs italic">
                  {cert.note}
                </p>
              )}
            </div>
        ...
        <div className="mt-10 mb-4 grid gap-4 sm:grid-cols-2">
          <Link
            href="/partners"
            className="border-line bg-card hover:border-gold-text block rounded-xl border p-5 transition-colors"
          >
            <h3 className="font-heading mb-1 text-base font-medium">Партнёрская сеть</h3>
            <p className="text-muted-foreground text-sm">
              Сертифицированные кухни для событий 500+ гостей. Аудит: медкнижки, ХАССП, страхование
              ГО.
            </p>
            <p className="text-gold-text mt-2 text-xs font-semibold">Смотреть партнёров →</p>
          </Link>
          <Link
            href="/account/orders"
            className="border-line bg-card hover:border-gold-text block rounded-xl border p-5 transition-colors"
          >
            <h3 className="font-heading mb-1 text-base font-medium">Личный кабинет B2B</h3>
            <p className="text-muted-foreground text-sm">
              История заказов, закрывающие документы на скачивание, статусы событий. Для постоянных
              B2B-клиентов.
            </p>
            <p className="text-gold-text mt-2 text-xs font-semibold">Войти в ЛК →</p>
          </Link>
        </div>

        {/* Downloadable templates */}
        <div className="border-gold-tint bg-gold-tint/5 mt-4 rounded-xl border-2 p-6">
          <h2 className="font-heading mb-2 text-2xl font-medium">
            Шаблоны документов для скачивания
          </h2>
          <p className="text-muted-foreground mb-5 text-sm">
            Скачайте шаблоны прямо сейчас — без email-ожидания. Все шаблоны согласуются
            индивидуально под ваш контракт. Подпись — ЭДО (Диадок/СБИС) или оригинал с печатью.
          </p>
          <div className="grid gap-4 sm:grid-cols-3">
            <a
              href="/api/templates/dogovor"
              download="nilov-dogovor-template.pdf"
              className="border-line bg-card hover:border-gold-text block rounded-xl border p-4 transition-all hover:shadow-md"
            >
              <div className="mb-2 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-gold-text text-xs font-semibold">PDF · ~7 КБ</p>
                </div>
              </div>
              <p className="text-muted-foreground text-xs">
                Шаблон договора на оказание кейтеринговых услуг: предмет, обязанности, оплата,
                форс-мажор, реквизиты.
              </p>
            </a>
            <a
            ...
            >
              ...
              <p className="text-muted-foreground text-xs">
                Соглашение о неразглашении: список гостей, меню, бюджет, фото. Срок 3 года, фото —
                бессрочно.
              </p>
            </a>
            <a
            ...
            >
              ...
              <p className="text-muted-foreground text-xs">
                Уровень обслуживания: время реакции ≤2ч, тайминг события, штрафы за нарушение,
                эскалация.
              </p>
            </a>
          </div>
        ...
        {/* CTA */}
        <div className="bg-primary text-primary-foreground mt-10 rounded-xl p-6 text-center">
          <h2 className="font-heading mb-2 text-xl font-medium">Нужен полный пакет документов?</h2>
          <p className="mb-4 text-sm opacity-90">
            Для школ, учреждений, B2B: счёт, акт, счёт-фактура, медкнижки, бракеражный журнал,
            программа производственного контроля, полис страхования.
          </p>
