...
export const metadata: Metadata = {
  ...
  description:
    "Поминальный обед в Санкт-Петербурге. Постное меню без алкоголя: кутья, блины, кисель, рыбные блюда. Доставка в кафе и домой. Эконом 1 800 ₽/гость или Стандарт 2 500 ₽/гость.",
};
    ...
    <main id="main" className="pb-20" data-hide-newsletter="true">
      ...
      <section className="relative flex min-h-[60vh] items-end overflow-hidden md:min-h-[70vh]">
        ...
        <div className="relative z-10 w-full">
          <div className="container-site max-w-5xl pt-28 pb-10 md:pt-32 md:pb-14">
            ...
            <p className="text-gold-soft mb-3 text-[11px] tracking-[0.25em] uppercase md:text-xs">
            ...
            <p className="mb-6 max-w-2xl text-base leading-relaxed text-white/85 md:text-lg">
              Тихо, достойно, по православной традиции. Постное меню, кутья, блины, кисель. Доставка
              в кафе, храм или домой — в пределах СПб.
            </p>

            <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-white md:text-base">
              <span className="font-semibold">
                <span className="text-gold-text">1 800 ₽</span>/гость{" "}
                <span className="text-muted-foreground text-xs">
                  (Эконом, фикс.) · 2 500 ₽/гость (Стандарт)
                </span>
              </span>
      ...
      <section className="border-line bg-card border-b">
        <div className="container-site max-w-5xl">
          <div className="divide-line grid grid-cols-2 divide-x md:grid-cols-4">
            {QUICK_FACTS.map((f) => (
              <div key={f.label} className="px-4 py-5 text-center">
                ...
                <div className="text-muted-foreground mt-1 text-xs md:text-sm">{f.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="container-site max-w-5xl">
        {/* ───────────────── INTRO ───────────────── */}
        <RevealBlock className="mt-12 mb-10 md:mt-16">
          <div className="max-w-3xl">
            <h2 className="font-heading mb-4 text-2xl font-medium md:text-3xl">
              Особый вид мероприятия
            </h2>
            <p className="text-foreground/85 mb-4 text-base leading-relaxed md:text-lg">
              Поминальный обед — не банкет и не фуршет. Мы готовим по православной традиции: постное
              меню без алкоголя, без торта «С днём рождения», без шумных тостов. Кутья, блины,
              кисель — обязательно. Доставка в кафе, в церковный зал, в кафе рядом с кладбищем или
              домой. Тихо, достойно, профессионально.
            </p>
            <div className="mt-5">
              <ShareButton
        ...
        <RevealBlock className="mb-14 md:mb-20">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3 md:gap-5">
            {WHATS_INCLUDED.map((item) => (
              <article
                key={item.title}
                className="border-line bg-card hover:border-foreground/30 overflow-hidden rounded-xl border transition-colors"
              >
                <FoodPhoto
                  src={item.photo}
                  alt={item.alt}
                ...
                <div className="p-4 md:p-5">
                  <h3 className="font-heading mb-1.5 text-base font-medium md:text-lg">
                    {item.title}
                  </h3>
        ...
        <RevealBlock className="mb-14 md:mb-20">
          <div className="border-line bg-secondary/40 overflow-hidden rounded-2xl border">
            <div className="grid grid-cols-1 gap-0 md:grid-cols-3">
              {POMINKI_MENU[0].items.map((item, idx) => {
                ...
                return (
                  <div
                    key={item.name}
                    className={`relative ${idx > 0 ? "border-line md:border-l" : ""}`}
                  >
                    <FoodPhoto
                      src={img}
                      alt={item.name}
                    ...
                    />
                    <div className="p-5">
                      <h3 className="font-heading mb-1 text-lg font-medium">{item.name}</h3>
                      <p className="text-muted-foreground mb-2 text-sm leading-relaxed">
                        {item.desc}
                      </p>
                      <p className="text-foreground/80 text-sm font-semibold">{item.price}</p>
                    </div>
                  </div>
                );
              })}
        ...
        <RevealBlock className="mb-10">
          <div className="mb-6 flex flex-wrap items-end justify-between gap-2">
            <h2 className="font-heading text-2xl font-medium md:text-3xl">Поминальное меню</h2>
            <p className="text-muted-foreground text-sm">{totalMin} блюд · фикс-цена</p>
          </div>
        </RevealBlock>

        <div className="mb-14 space-y-10 md:mb-20 md:space-y-12">
          {POMINKI_MENU.slice(1).map((sec, secIdx) => (
            <RevealBlock key={sec.section} delay={secIdx * 0.05}>
              <section>
                <div className="mb-4 flex flex-wrap items-baseline justify-between gap-2 md:mb-5">
                  <h3 className="font-heading text-foreground text-xl font-medium md:text-2xl">
                    {sec.section}
                  </h3>
                  {sec.note && (
                    <p className="text-muted-foreground max-w-md text-xs italic md:text-sm">
                      {sec.note}
                    </p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-5 lg:grid-cols-3">
                  {sec.items.map((item) => {
                    ...
                    return (
                      <article
                        key={item.name}
                        className="border-line bg-card hover:border-foreground/30 overflow-hidden rounded-xl border transition-all hover:shadow-[0_4px_12px_rgba(28,24,21,0.05)]"
                      >
                        <div className="relative">
                          <FoodPhoto
                            ...
                            objectPosition={pos}
                          />
                          <span className="text-foreground absolute top-2 right-2 rounded-md bg-white/95 px-2 py-1 text-xs font-semibold shadow-sm">
                            {item.price}
                          </span>
                        </div>
                        <div className="p-4">
                          <h4 className="mb-1 text-sm font-medium md:text-base">{item.name}</h4>
        ...
        <RevealBlock className="mb-14 md:mb-20">
          <div className="border-line bg-card rounded-2xl border p-6 md:p-8">
            <h2 className="font-heading mb-3 text-xl font-medium md:text-2xl">Пример расчёта</h2>
            <p className="text-foreground/85 mb-5 text-sm leading-relaxed md:text-base">
              <strong>1 800 ₽/гость — фиксированная цена.</strong> Включает: еда, посуда, доставка
              по СПб (в пределах КАД), салфетки, столовые приборы. Доплаты: выезд за КАД (+30 ₽/км),
              дополнительный персонал на площадке (+1 500 ₽/час). Минимум — 10 человек.
            </p>
            <div className="bg-secondary/60 rounded-lg p-4 md:p-5">
              <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 text-sm md:text-base">
                ...
                <span className="text-muted-foreground">=</span>
                <span className="font-heading text-foreground text-2xl font-semibold md:text-3xl">
                  45 000 ₽
                </span>
              </div>
        ...
        <RevealBlock className="mb-14 md:mb-20">
          <div className="mb-6 md:mb-8">
            ...
            <h2 className="font-heading text-2xl font-medium md:text-3xl">Как мы сервируем</h2>
            <p className="text-muted-foreground mt-2 max-w-2xl text-sm md:text-base">
              Поминки — это серьёзное событие. Мы соблюдаем православную традицию и не предлагаем
              того, что неуместно.
            </p>
          </div>

          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-5">
            <div className="border-line bg-card rounded-xl border p-5 md:p-6">
              <h3 className="font-heading text-foreground mb-4 text-base font-medium md:text-lg">
                Что входит
              </h3>
              <ul className="text-foreground/90 space-y-2.5 text-sm">
                {PROTOCOL_DO.map((line) => (
                  <li key={line} className="flex gap-2.5">
                    <span
                      className="bg-foreground/60 mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full"
                      aria-hidden="true"
                    />
            ...
            <div className="border-line bg-card rounded-xl border p-5 md:p-6">
              ...
              <ul className="text-foreground/90 space-y-2.5 text-sm">
                {PROTOCOL_DONT.map((line) => (
                  <li key={line} className="flex gap-2.5">
                    <span
                      className="bg-destructive/70 mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full"
                      aria-hidden="true"
                    />
                    <span className="leading-relaxed">{line}</span>
                  </li>
          ...
          <div className="border-line bg-secondary/40 mt-5 rounded-xl border p-5 md:p-6">
            <p className="text-foreground/90 text-sm leading-relaxed md:text-base">
              <strong className="font-medium">Доставка рядом с кладбищами:</strong> Богословское,
              Смоленское, Ковалёвское, Северное — согласуем с менеджером по телефону. Срочные
              поминки возможны — даже на день обращения, если есть свободная бригада.
            </p>
          </div>
        </RevealBlock>
        ...
        {/* ───────────────── КАК ЗАКАЗАТЬ ───────────────── */}
        <RevealBlock className="mb-14 md:mb-20">
          <div className="border-foreground/20 bg-card rounded-2xl border p-6 md:p-8">
            <h2 className="font-heading mb-4 text-xl font-medium md:text-2xl">Как заказать</h2>
            <ol className="text-foreground/90 mb-5 space-y-3 text-sm md:text-base">
              <li className="flex gap-3">
                <span className="font-heading text-foreground/60 shrink-0 font-semibold">1.</span>
                <span>
                  Позвоните:{" "}
                  <a
                    href="tel:+78129195911"
                    className="text-foreground hover:text-muted-foreground font-semibold underline underline-offset-2"
                  >
                    +7 (812) 919-59-11
                  </a>
              ...
              <li className="flex gap-3">
                <span className="font-heading text-foreground/60 shrink-0 font-semibold">2.</span>
                <span>
                  Скажите: дату, время, количество гостей, адрес (дом, кафе или кладбище).
                </span>
              </li>
              <li className="flex gap-3">
                <span className="font-heading text-foreground/60 shrink-0 font-semibold">3.</span>
                <span>Менеджер предложит меню под ваш бюджет — фикс 1 800 ₽/гость.</span>
              </li>
              <li className="flex gap-3">
                <span className="font-heading text-foreground/60 shrink-0 font-semibold">4.</span>
                <span>Курьер привезёт за 60 минут до начала. Сервируем тихо.</span>
              </li>
            </ol>

            <div className="bg-secondary/60 mb-5 rounded-lg p-4 md:p-5">
              <p className="text-foreground/90 text-sm leading-relaxed md:text-base">
                ...
                . Менеджер свяжется в течение 15 минут. Можно общаться только по email/ЭДО — укажите
                это в форме заявки.
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-3 text-sm">
              <a
                href="tel:+78129195911"
                className="bg-primary text-primary-foreground hover:bg-primary/90 inline-flex items-center gap-2 rounded-lg px-5 py-3 text-sm font-semibold transition-colors"
              >
                +7 (812) 919-59-11
              </a>
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="border-line text-foreground hover:border-foreground/40 inline-flex items-center gap-2 rounded-lg border px-5 py-3 text-sm font-semibold transition-colors"
              >
                WhatsApp
              </a>
              <Link
        ...
        {/* ───────────────── CTA ───────────────── */}
        <RevealBlock>
          <div className="border-line bg-card/50 rounded-2xl border border-dashed p-6 text-center md:p-8">
            <p className="font-heading mb-2 text-lg font-medium md:text-xl">
              Принимаем срочные заказы
            </p>
            <p className="text-muted-foreground mx-auto max-w-xl text-sm md:text-base">
              Даже на день обращения, если есть свободная бригада. Позвоните — менеджер ответит за
              15 минут.
            </p>
          </div>
        </RevealBlock>
