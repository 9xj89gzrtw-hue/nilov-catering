 ...
 */
function getLocationsForOccasion(
  occasion: string | undefined
): readonly { label: string; emoji: string }[] {
...
function getRecommendation(
  ...
  switch (occasion) {
      ...
      if (small)
        return {
          ...
          whatIncluded: [
            ...
            "Вино (1 бокал) + игристое на welcome",
            "Официант 1/10",
      ...
      if (med)
        return {
          ...
          whatIncluded: [
            ...
            "Рыбное блюдо + сырная тарелка",
            "Вино (2 бокала) + игристое",
            "Свадебный торт + десерты",
    ...
    case "Корпоратив":
      if (small)
        return {
          name: "Корпоратив «Фуршет Эконом»",
          price: 2450,
          minGuests: 20,
          tier: "Эконом",
          format: "furshet",
          whatIncluded: [
            "Канапе (6 видов), тарталетки, брускетты",
            "Сырная тарелка, рыбное ассорти",
            "Соки, морсы, лимонады",
            "Официант 1/15",
      ...
      if (med)
        return {
          name: "Корпоратив «Банкет Стандарт»",
          price: 5470,
          minGuests: 15,
          tier: "Стандарт",
          format: "banket",
          whatIncluded: [
            "2 холодные закуски, салат, горячее",
            "Говядина/лосось на выбор",
            "Вино (1 бокал), чай/кофе",
            "Официант 1/10",
            "Координатор",
          ],
        };
      return {
        ...
        whatIncluded: [
          ...
          "Карпаччо, сыры, морепродукты",
          "Игристое, вино, коктейли",
          "Официант 1/12",
          "Шоу-станция (опц.)",
        ],
      };
    case "День рождения":
      if (small)
        return {
          name: "День рождения «Фуршет Эконом»",
          price: 2450,
          minGuests: 20,
          tier: "Эконом",
          format: "furshet",
          whatIncluded: [
            "Канапе (4 вида), тарталетки",
            "Мини-бургеры, овощная нарезка",
      ...
      return {
        name: "День рождения «Фуршет Стандарт»",
        price: 3950,
        minGuests: 20,
        tier: "Стандарт",
        format: "furshet",
    ...
    case "Детский праздник":
      return {
        name: "Детский праздник «Эконом»",
        price: 1550,
        minGuests: 10,
        tier: "Эконом",
        format: "detskoe",
        whatIncluded: [
          "Мини-пицца, наггетсы, канапе без аллергенов",
          "Фруктовая тарелка, соки",
          "Капкейки, мармелад",
          "Аниматор (опц.)",
          "Посуда яркая, небьющаяся",
        ],
      };
    case "Выпускной":
      if (small)
        return {
          name: "Выпускной «Банкет Эконом»",
          price: 3950,
          minGuests: 15,
          tier: "Эконом",
          format: "banket",
          whatIncluded: [
            "Холодная закуска, салат, горячее",
            "Курица/рыба на выбор",
            "Гарнир, хлебная корзина",
            "Чай/кофе",
            "Официант 1/10",
          ],
        };
      return {
        name: "Выпускной «Банкет Стандарт»",
        price: 5470,
        minGuests: 15,
        tier: "Стандарт",
        format: "banket",
...
export default async function PlanHelperPage({
    ...
    return (
      <main id="main" className="pt-24 pb-20">
        <div className="container-site mx-auto max-w-2xl">
          ...
          {recommendation && (
            <div className="border-gold-text bg-card shadow-gold/10 mb-6 rounded-xl border-2 p-6 shadow-lg">
              <div className="mb-3 flex flex-wrap items-baseline justify-between gap-2">
                <h2 className="font-heading text-xl font-medium">{recommendation.name}</h2>
                <span className="bg-gold-text inline-block rounded-full px-2 py-0.5 text-xs font-semibold text-white">
                  {recommendation.tier}
                </span>
              </div>
              <div className="mb-4 flex items-baseline gap-1">
                ...
                <span className="text-muted-foreground text-sm">
                  ₽/гость · мин. {recommendation.minGuests} гостей
                </span>
              </div>
              <p className="text-muted-foreground mb-3 text-sm">
                Формат: <strong className="text-foreground">{formatLabel}</strong>. Подобрали под
                ваш повод и количество гостей.
              </p>
          ...
          <div className="border-line bg-card mb-6 rounded-xl border p-6">
            <h2 className="font-heading mb-3 text-xl font-medium">Что дальше</h2>
            <div className="grid gap-3 sm:grid-cols-2">
              <Link
                href={eventPage}
                className="bg-primary text-primary-foreground hover:bg-primary/90 rounded-lg px-5 py-3 text-center text-sm font-semibold no-underline transition-colors"
              >
                📄 Открыть страницу события
              </Link>
              <Link
                href={`/plan/constructor?format=${format}`}
                className="border-line bg-background hover:border-gold-text rounded-lg border px-5 py-3 text-center text-sm font-semibold no-underline transition-colors"
              >
          ...
          <div className="text-center">
            <a
              ...
              className="border-gold-text text-foreground hover:bg-gold-tint/10 mr-3 inline-block rounded-lg border-2 px-6 py-3 text-base font-semibold no-underline transition-colors"
            >
              📞 {SITE.phone}
            </a>
            <Link
              href="/plan/helper"
              className="text-muted-foreground hover:text-foreground mt-2 inline-block text-sm"
            >
              ↺ Начать заново
            </Link>
          </div>
  ...
  const current =
    step === 0
      ? {
          q: "Какой повод?",
          opts: OCCASIONS.map((o) => ({ label: o.label, emoji: o.emoji })),
          key: "occasion" as const,
        }
      : step === 1
        ? {
            q: "Сколько гостей?",
            opts: GUEST_RANGES.map((g) => ({ label: g.label, emoji: "" })),
            key: "guests" as const,
          }
        : {
            q: "Где проходит?",
            opts: locations.map((l) => ({ label: l.label, emoji: l.emoji })),
        ...
        <div className="space-y-3">
          {current.opts.map((opt) => (
            <Link
              ...
              className={`block w-full rounded-xl border p-5 text-left no-underline transition-all ${
                step === 0 && occasion === opt.label
                  ? "border-gold-text bg-gold-tint ring-gold-text text-foreground ring-1"
            ...
            >
              {opt.emoji && <span className="mr-3 text-2xl">{opt.emoji}</span>}
          ...
          <Link
            href={
              step === 1
                ? "/plan/helper"
                : step === 2
                  ? `/plan/helper?occasion=${encodeURIComponent(occasion!)}`
                  : "/plan/helper"
            }
            className="text-muted-foreground hover:text-foreground mt-6 inline-block text-sm"
          >
            ← Назад
          </Link>
        )}

        <div className="bg-secondary/50 text-muted-foreground mt-12 rounded-lg p-4 text-center text-sm">
