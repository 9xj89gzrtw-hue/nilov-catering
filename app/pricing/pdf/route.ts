...
// PDF-specific CSS values (explicit pixels required for PDF generation)
const PDF_COL_WIDTH_DISH = "width:40%;";
const PDF_COL_WIDTH_QTY = "width:80px;";
const PDF_TABLE_STYLE =
  "width: 100%; border-collapse: collapse; margin-top: 8px;"; /* eslint-disable-line -- PDF table style */
  ...
  for (const [eventId, offers] of Object.entries(ALL_TARIFF_OFFERS)) {
    ...
    for (const offer of offers) {
      rows.push(`<div class="tier">
        ...
        <table>
          <thead><tr><th style=${PDF_COL_WIDTH_DISH}>Блюдо</th><th>Состав</th><th style=${PDF_COL_WIDTH_QTY}>Кол-во</th></tr></thead>
          <tbody>
            ${offer.composition
              .map(
                (item) => `<tr>
