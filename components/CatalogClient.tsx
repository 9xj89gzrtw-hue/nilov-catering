    ...
    <div className="drinqit-3d drinqit-shine rounded-xl border border-line bg-card overflow-hidden group hover:border-gold-text transition-all duration-300 hover:shadow-xl flex flex-col">
      ...
      <Link href={constructorHref} className="relative block drinqit-3d-img" aria-label={`${dish.name} — открыть в конструкторе меню`}>
        ...
        <button
          ...
          className={`absolute top-2 right-2 z-10 w-10 h-10 rounded-full backdrop-blur-md shadow-lg flex items-center justify-center transition-all ${
            isInCart
              ? 'bg-success text-white opacity-100'
              : 'bg-white/90 text-foreground opacity-0 group-hover:opacity-100 hover:bg-white hover:scale-110'
          }`}
      ...
      <div className="p-3 flex-1 flex flex-col">
        ...
        <div className="mt-auto flex gap-2">
          <button
            onClick={handleAddToCart}
            className={`flex-1 inline-flex items-center justify-center rounded-lg px-3 py-2 text-xs font-semibold transition-all touch-target ${
              isInCart
