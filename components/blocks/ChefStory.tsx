...
export default function ChefStory() {
  ...
  return (
    <section 
      ref={ref}
      className="py-20 md:py-32 bg-secondary/40 relative overflow-hidden" 
      aria-labelledby="chef-story-heading"
    >
      {/* Decorative background elements */}
      <div 
        className="absolute top-20 left-10 w-[400px] h-[400px] rounded-full opacity-[0.04]"
        style={{ background: 'radial-gradient(circle, #D4AF37 0%, transparent 70%)' }}
        aria-hidden="true"
      />
      <div 
        className="absolute bottom-10 right-10 w-[300px] h-[300px] rounded-full opacity-[0.03]"
        style={{ background: 'radial-gradient(circle, #D4AF37 0%, transparent 70%)' }}
        aria-hidden="true"
      />
      ...
      <div 
        ...
        style={{
          background: 'linear-gradient(90deg, transparent 0%, #C9A961 30%, #D4AF37 50%, #C9A961 70%, transparent 100%)',
        }}
        aria-hidden="true"
      />
      ...
      <div className="container-site relative z-10">
        ...
        >
          ...
          >
            История{' '}
            <span className="text-gold-premium">шефа</span>
          </h2>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-start max-w-6xl mx-auto">
          ...
          <motion.div
            ...
            className="relative"
          >
            {/* Art-deco corner accents container */}
            <div className="relative max-w-md mx-auto lg:mx-0">
              {/* Top-left corner */}
              <div 
                className="absolute -top-4 -left-4 w-12 h-12 border-t-2 border-l-2 border-[#D4AF37]/50 rounded-tl-xl z-10"
                aria-hidden="true"
              />
              {/* Bottom-right corner */}
              <div 
                className="absolute -bottom-4 -right-4 w-12 h-12 border-b-2 border-r-2 border-[#D4AF37]/50 rounded-br-xl z-10"
                aria-hidden="true"
              />
              ...
              {/* Floating experience badge — premium styling */}
              <div 
                className="absolute -right-3 -bottom-3 md:-right-6 md:-bottom-6 px-5 py-3.5 rounded-xl shadow-lg z-20"
                style={{
          ...
          >
            ...
            {/* Quote block — premium styling with gold gradient border */}
            <blockquote 
              className="relative p-6 md:p-8 rounded-2xl bg-card overflow-hidden"
              style={{
            ...
            >
              ...
              <span 
                className="absolute top-3 left-4 text-7xl md:text-8xl leading-none select-none opacity-10"
                style={{ 
        ...
        {/* Achievements badges — enhanced grid */}
        <motion.div 
        ...
        >
          <h3 className="font-heading text-lg md:text-xl text-center mb-9" style={{ fontWeight: 500 }}>
            Вехи карьеры
          </h3>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
            {ACHIEVEMENTS.map((item) => (
              <div
                key={item.title}
                className="group flex items-start gap-3 p-4 md:p-5 rounded-xl bg-card border border-line hover:border-[#D4AF37]/40 transition-all duration-300"
                style={{
                  boxShadow: '0 1px 3px rgba(28,24,21,0.04)',
                }}
                onMouseEnter={(e) => {
                  e.currentTarget.style.boxShadow = '0 4px 16px rgba(139,105,20,0.08), 0 0 0 1px rgba(212,175,55,0.1)';
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.boxShadow = '0 1px 3px rgba(28,24,21,0.04)';
                }}
              >
                <div 
                  className="shrink-0 w-10 h-10 rounded-lg flex items-center justify-center transition-colors duration-300 group-hover:bg-[#D4AF37]/15"
                  style={{ background: 'rgba(239,230,214,0.6)' }}
                >
                  <item.icon 
                    className="w-5 h-5 text-gold-text group-hover:text-[#B8860B] transition-colors" 
                    strokeWidth={1.5} 
                    aria-hidden="true" 
                  />
                </div>
        ...
        {/* CTA Section — premium dark block */}
        <motion.div 
        ...
        >
          <div 
            ...
            style={{
              background: 'linear-gradient(145deg, #1C1815 0%, #2A2420 100%)',
            }}
          >
            {/* Decorative pattern */}
            <div 
            ...
            <div className="relative z-10">
              <p className="text-xs uppercase tracking-[0.22em] text-[#D4AF37] mb-3 font-medium">
                От первого лица
              </p>
              <h3 className="font-heading text-2xl md:text-3xl mb-3 text-white" style={{ fontWeight: 500 }}>
                Хотите попробовать?
              </h3>
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center items-stretch sm:items-center">
                <Link
                  href="/menu"
                  className="group inline-flex items-center justify-center gap-2.5 rounded-full px-8 py-4 text-base font-semibold no-underline min-w-[240px] transition-all duration-300 hover:-translate-y-0.5"
                  style={{
                ...
                <Link
                  href="/tasting"
                  className="inline-flex items-center justify-center gap-2.5 rounded-full border-2 border-white/20 text-white px-6 py-4 text-base font-medium no-underline min-w-[200px] hover:bg-white/5 hover:border-white/30 transition-all duration-300"
                >
      ...
      <div 
        ...
        style={{
          background: 'linear-gradient(90deg, transparent, #C9A961 30%, #D4AF37 50%, #C9A961 70%, transparent 100%)',
        }}
        aria-hidden="true"
      />
    </section>
