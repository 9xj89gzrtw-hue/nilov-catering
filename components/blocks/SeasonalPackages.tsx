  ...
  "new-year": [
    {
      ...
      items: [
        ...
        },
        {
          ...
          desc: "Икра лососёвая, сливочное масло, багет",
          qty: "2 шт/гость",
        },
        {
          name: "Тарталетка с крабом",
          desc: "Крабовое мясо, сливочный сыр, авокадо",
          qty: "1 шт/гость",
        },
        { name: "Запечённый гусь", desc: "Гусь с яблоками и черносливом", qty: "1 порция" },
        {
          name: "Корюшка горячего копчения",
          desc: "Корюшка, копчёная на ольховой щепе",
          qty: "3 шт/гость",
        },
        {
          name: "Мандариновый десерт",
          desc: "Мусс из мандаринов, бисквит, цедра",
          qty: "1 стакан",
        },
        {
          name: "Имбирный чай с облепихой",
          desc: "Облепиха, имбирь, мёд, корица",
          qty: "безлимит",
        },
        {
          name: "Глинтвейн безалкогольный",
          desc: "Виноградный сок, пряности, апельсин",
          qty: "1 бокал",
        },
      ],
    },
    {
      ...
      items: [
        {
          name: "Оливье с раковыми шейками",
          desc: "Раковые шейки, картофель, яйцо, горошек",
          qty: "1 порция",
        },
    ...
    },
    {
      ...
      items: [
        {
          name: "Оливье с раковыми шейками",
          desc: "Раковые шейки, картофель, яйцо",
          qty: "1 порция",
        },
  ...
  maslenitsa: [
      ...
      items: [
        {
          ...
          desc: "Тонкие блины со сметаной, мёдом, вареньем",
          qty: "3 шт/гость",
        },
        {
        ...
        {
          name: "Блины с сёмгой",
          desc: "Слабосолёная сёмга, творожный сыр, укроп",
          qty: "1 шт/гость",
        },
        {
        ...
        {
          name: "Блины с мясом",
          desc: "Томлёная говядина, лук, сметанный соус",
          qty: "1 шт/гость",
        },
    ...
    },
    {
      ...
      items: [
        ...
        {
          name: "Блины с красной икрой",
          desc: "Икра лососёвая, сливочное масло",
          qty: "2 шт/гость",
        },
        ...
        {
          name: "Оладьи яблочные",
          desc: "Пышные оладьи, карамелизированные яблоки",
          qty: "2 шт/гость",
        },
      ...
      items: [
        {
          ...
          desc: "Свиная шея в луковом маринаде, 200 г",
          qty: "1 порция",
        },
    ...
    },
    {
      ...
      items: [
        ...
        { name: "Шашлык из курицы", desc: "Бедро в аджике, 200 г", qty: "1 порция" },
        {
          ...
          desc: "Рубленая баранина с зеленью, 150 г",
          qty: "1 порция",
        },
...
export default function SeasonalPackages({ season, label, ctaFormat }: Props) {
  ...
  return (
    <section className="mb-12">
      <h2 className="font-heading mb-1 text-xl font-medium">Готовые пакеты: {label}</h2>
      <p className="text-muted-foreground mb-6 text-sm">
        Выберите готовый пакет или{" "}
        <Link
          href={ctaFormat ? `/plan/constructor?format=${ctaFormat}` : "/plan/constructor"}
...
function PkgBlock({ pkg }: { pkg: Pkg }) {
  ...
  return (
    <div className="border-line bg-card overflow-hidden rounded-2xl border-2">
      <div className="p-5">
        <div className="mb-2 flex items-start justify-between">
          <h3 className="font-heading text-lg font-semibold">{pkg.name}</h3>
          <span className="text-gold-text ml-4 shrink-0 text-lg font-bold">{pkg.price}</span>
        </div>
        <p className="text-muted-foreground mb-3 text-sm">{pkg.desc}</p>

        <button
          onClick={() => setOpen(!open)}
          className="bg-secondary/30 hover:bg-secondary/50 flex w-full items-center justify-between rounded-lg px-3 py-2 text-sm transition-colors"
        >
          <span>
            {open ? "Скрыть состав" : "Показать состав"} ({pkg.items.length} позиций)
          </span>
          <svg
        ...
        {open && (
          <div className="mt-4 space-y-1.5">
            {pkg.items.map((item, j) => (
              <div key={j} className="border-line/20 border-b py-1.5 last:border-0">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <span className="text-sm font-medium">{item.name}</span>
                    <p className="text-muted-foreground mt-0.5 text-xs leading-snug">{item.desc}</p>
                  </div>
                  <span className="text-muted-foreground mt-0.5 shrink-0 text-xs">{item.qty}</span>
                </div>
              </div>
            ))}
          </div>
