...
// Unified trust strip — заменил отдельный TrustBar блок на главной.
// Раньше: TrustBar (marquee из 8 кейсов) + TestimonialsCarousel (слайдер отзывов).
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
        {TRUST_METRICS.map(m => (
          <div key={m.label} className="text-center">
            <p className="font-heading text-2xl md:text-3xl font-bold text-gold-text">{m.value}</p>
  ...
  return (
    <section className="py-16 md:py-20 bg-secondary" aria-labelledby="reviews-heading">
      <div className="container-site max-w-lg mx-auto text-center">
        ...
        <Link href="/reviews" className="text-sm font-medium text-gold-text hover:underline">Все отзывы и кейсы →</Link>
      </div>
    </section>
  );
}
