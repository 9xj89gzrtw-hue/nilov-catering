 ...
 */
// Mobile breakpoint for marquee behavior (/* eslint-disable-line -- media query breakpoint */)
const MOBILE_BREAKPOINT = "(max-width: 767px)"; /* eslint-disable-line -- media query */
...
export function TrustMarquee({ clients }: TrustMarqueeProps) {
  ...
  const doubled = useMemo(() => [...visibleClients, ...visibleClients], [visibleClients]);
  ...
  const renderCard = (client: ClientCard, i: number) => {
    const content = (
      <>
        <p className="text-foreground mb-1 text-center text-xs font-medium tracking-wider md:text-sm">
          {client.name}
        </p>
        {client.ref && (
        ...
        {client.status === "verified" && <p className="mt-1 text-[9px] text-[#065F46]">проверен</p>}
      </>
    );
    ...
    if (client.ref?.reviewId) {
      return (
        <Link key={`${client.id}-${i}`} href={`/reviews#${client.ref.reviewId}`} className={cls}>
          {content}
        </Link>
      );
    }
