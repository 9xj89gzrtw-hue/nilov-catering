...
export default function Breadcrumbs() {
  ...
  return (
    <nav aria-label="Хлебные крошки" className="text-foreground/80 mb-6 text-base">
      <script
      ...
      <ol className="flex flex-wrap items-center gap-1.5">
        {crumbs.map((crumb) => (
          <li key={crumb.href} className="flex items-center gap-1.5">
            <Link
              href={crumb.href!}
              className="text-foreground/70 hover:text-gold-text no-underline transition-colors"
            >
              {crumb.label}
            </Link>
