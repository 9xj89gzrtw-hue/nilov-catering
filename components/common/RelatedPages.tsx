...
export default function RelatedPages({ context, slug, title, maxLinks = 4 }: RelatedPagesProps) {
  ...
  return (
    <section
      className="border-line bg-secondary/30 mt-16 rounded-2xl border p-6 md:p-8"
      aria-labelledby={`related-${context}-${slug}`}
    >
      <h2
        className="font-heading mb-6 text-xl font-medium md:text-2xl"
          ...
          <Link
            ...
            className={`group flex items-start gap-3 rounded-xl p-4 transition-all duration-200 hover:-translate-y-0.5 ${
              link.cta
                ...
                : "bg-card hover:border-gold-text/30 border border-transparent hover:shadow-sm"
            }`}
          >
            {/* Иконка */}
            {link.icon && (
            ...
            <div className="min-w-0 flex-1">
              ...
              >
                {link.label}
                {link.cta && <ArrowRight className="h-4 w-4" aria-hidden="true" />}
              </div>
              {link.desc && (
                <p
    ...
    <Link
      ...
      className={`group flex items-center gap-4 rounded-2xl p-6 transition-all duration-300 hover:-translate-y-1 ${
        variant === "primary"
          ...
          : "bg-card hover:border-gold-text border hover:shadow-md"
      }`}
    >
      {icon && (
        <span
      ...
      <div className="flex-1">
        ...
        <p
          className={`mt-0.5 text-sm ${variant === "primary" ? "text-white/80" : "text-muted-foreground"}`}
        >
          {desc}
        </p>
      </div>
