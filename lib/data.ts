 ...
 * Работаем с юрлицами по безналичному расчёту — выставляем счёт, акт, УПД (через партнёрское ООО при необходимости).
...
export const LEGAL: {
  ...
  vatStatus:
    "Работаем по безналу с юрлицами на УСН (без НДС). Для клиентов на ОСН, требующих счёт-фактуру с НДС, партнёрское ООО выставляет документы — уточняйте у менеджера.",
  legalAddress:
    "199106, г. Санкт-Петербург, Василеостровский район, 20-я линия, дом 11, помещение 5-Н",
  edo: {
    diadoc: "2AE (Контур.Диадок)",
    sbis: "2АК (СБИС)",
  },
  bank: {
    name: "Филиал №7801 ПАО «Банк ВТБ» в г. Санкт-Петербурге",
    bik: "044030706",
...
export function formatEarlyBirdDiscounts(): string {
  return EARLY_BIRD_DISCOUNTS.slice(1) // skip 5% for short form (most pages show only 10/15)
...
export function formatEarlyBirdDiscountsFull(): string {
  return EARLY_BIRD_DISCOUNTS.map((d) => `${d.label} за ${d.days}+ дней`).join(", ");
}
