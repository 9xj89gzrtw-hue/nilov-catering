  ...
  async headers() {
    const securityHeaders = [
      ...
      { key: 'Permissions-Policy', value: 'camera=(), microphone=(), geolocation=()' },
      {
        ...
        value: "default-src 'self'; script-src 'self' 'unsafe-inline' https://mc.yandex.ru https://web.archive.org; style-src 'self' 'unsafe-inline'; img-src 'self' data: https: blob:; font-src 'self' data:; connect-src 'self' https: wss:; frame-ancestors 'none'; base-uri 'self'; form-action 'self'",
      },
    ];
    return [
      {
