...
**Remove the destructive `store.clearItems()` call in `ConstructorWizard.tsx:135‑137`**. Replace with: if `?fromCart=1` is present, load cart as‑is. If `?dish=X` is present without `?fromCart=1`, *append* X to the existing cart instead of clearing.
...
```tsx
<Link href={constructorHref} className="mt-auto ... bg-gold-text ...">
...
{isInCart ? (
  <div className="mt-auto flex items-center gap-2">
    ...
    <span className="flex-1 text-center">{currentQty} порц · {currentQty * dish.pricePerGuest}₽</span>
    <button onClick={() => setItemQty(dish.id, currentQty + 1)}>+</button>
  </div>
) : (
  <button
    onClick={() => { addDish(dish.id); flyToCart(dish.id); }}
    className="mt-auto ... bg-gold-text ..."
  >
    Добавить в меню
  </button>
)}
...
<section className="mb-10">
  <h2 className="font-heading text-2xl mb-4">Готовые сеты</h2>
...
export const NAMED_SETS: NamedSet[] = [
  {
    ...
    composition: ['canape-salmon', 'bruschetta-tomato', 'tartaletka-olivier', 'mini-burger', 'cranberry-mors'],
...
export const FOOD_STATIONS: FoodStation[] = [
  { id: 'hamonera', label: 'Хамонера', emoji: '🥩', description: 'Нарезка хамона резцом на глазах у гостей', pricePerEvent: 12000 },
  // ... 10–12 more
];
```
...
**Week 2 — add the "browse like a shop" layer**
6. Add `NAMED_SETS` data + `NamedSetGrid` section.
7. Add `servingsPerGuest` + `weightGrams` to DishCard UI.
8. Add `TierComparisonRow` block.
9. Mobile bottom sheet cart (spring physics).
10. Fly‑to‑cart Framer Motion animation.

**Week 3 (polish)**
11. `FoodStationGrid` for named stations.
12. `CrossSellServices` block.
13. `MenuQuiz` modal "Подобрать за 2 минуты".
14. Saved‑menu shareable URLs (`/menu/saved/{id}`).
15. `ComparisonTray` for side‑by‑side dish comparison.
