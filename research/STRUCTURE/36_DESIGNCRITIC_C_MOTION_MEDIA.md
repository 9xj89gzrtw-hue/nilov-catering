...
- Где: `04_BLOCKS:382` — акцент «чувствуешь» = «_italic_ Cormorant»; НО `09:41` (B8) и `06_TOKENS:107` (B8) явно ЗАПРЕЩАЮТ Cyrillic-italic (faux-oblique выглядит дёшево) → акцент через `font-weight:600` + gold-text.
...
_Критик (C) — независимый, занижающий. Перепроверка метрик (Lighthouse/CWV/axe) — после сборки, отдельным объективным прогоном._
...
_Критик (C) — независимый, занижающий. Контр-примеры из реальных 2025–2026 бенчмарков выше.
Перепроверка метрик (Lighthouse/CWV/axe) — после сборки, отдельным объективным прогоном._
