...
2. **ПОТОМ** переименовать роуты + 301-редиректы (`next.config` или `middleware.ts`):
   ...
   - 🆕 `/plan/helper` (wizard «Я не знаю, что мне нужно — помогите» 🟡 G1/G5/G7 — точка входа новичка, см. `43_NAV_SPEC` §2; _[CoherenceChecker C11: унифицировано с `04:386`/`43` §2 — «помогите», не «помоги»]_)
